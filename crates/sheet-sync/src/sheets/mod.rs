...
//! end-to-end in tests via `FakeSheets` — no network, no CQL.
...
pub trait SheetsApi {
    ...
    /// Writes every cell edit in `edits` to `spreadsheet_id`. Implementations
    /// must fail loud (no partial write left silently in place) if any edit
    /// cannot be applied.
    fn write_cells(&self, spreadsheet_id: &str, edits: &[CellEdit]) -> anyhow::Result<()>;
}
...
impl SheetsApi for FakeSheets {
    ...
    fn write_cells(&self, _spreadsheet_id: &str, edits: &[CellEdit]) -> anyhow::Result<()> {
